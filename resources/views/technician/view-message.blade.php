...
@section('styles')
    ...
    <style>
        ...
        .reply-form .form-group .file-icon:hover {
            color: #E74A3A; /* Change color on hover */
        }
        ...
        .message-image {
            ...
            margin-top: 10px;
            display: block;
            max-height: 100%; /* Limit maximum height of customer images */
            border-radius: 5px; /* Add border radius to images */
        }
    </style>
@endsection
    ...
    <div class="card shadow mb-4">
        <div class="card-body d-flex flex-column"> <!-- Use flex-column to stack elements vertically -->
            ...
            <div class="row mb-4">
                ...
                <div class="col-12 col-md-2 mb-2 mb-md-0"><strong>Customer ID: </strong>{{ $message->customer->id }}</div>
                <div class="col-12 col-md-3 mb-2 mb-md-0"><strong>Subject: </strong>{{ $message->ticket->subject }}</div>
                <div class="col-12 col-md-3 mb-2 mb-md-0"><strong>Status: </strong>{{ $message->ticket->status }}</div>
                <div class="col-12 col-md-2 mb-2 mb-md-0"><strong>Date Posted: </strong>{{ $message->created_at->format('Y-m-d') }}</div>
                <div class="col-12 col-md-2 text-md-center">
                    <a href="{{ route('technician.message') }}" class="btn btn-secondary">Back</a>
                    <form action="{{ route('technician.delete-message', ['messageId' => $message->id]) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
            ...
            <div class="message-thread mb-auto"> <!-- Use 'mb-auto' to push to top -->
            ...
            <div class="reply-form mt-auto"> <!-- Use 'mt-auto' to push to bottom -->
                <form action="{{ route('technician.store-message') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="ticket_id" value="{{ $message->ticket->id }}">
