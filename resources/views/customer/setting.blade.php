        ...
        <div class="p-4">
            <form class="user" method="POST" action="{{ route('customer.update-settings') }}" enctype="multipart/form-data">
                ...
                <div class="text-center">
                    <img src="{{ asset('storage/' . $customer->profile_image) }}" id="profileImage" class="profile-image-1" alt="Profile Picture">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="mb-2 form-control" id="name" name="name" placeholder="Sothea" value="{{ old('name', $customer->name) }}">
                    @error('name')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="email" class="mb-2 form-control" id="email" name="email" placeholder="example@example.com" value="{{ old('email', $customer->email) }}">
                    @error('email')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group row">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control-sm" name="phone" id="phone" value="{{ old('phone', $customer->phone) }}">
                        @error('phone')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mt-3 form-group">
                        ...
                        <select id="nationality" name="nationality" class="form-control">
                            ...
                            @foreach($nationalities as $nationality)
                                <option value="{{ $nationality }}" {{ (old('nationality', $customer->nationality) == $nationality) ? 'selected' : '' }}>{{ ucfirst($nationality) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <br>
            <a href="{{ route('customer.dashboard') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Back</a>
        </div>
    </div>
@endsection
